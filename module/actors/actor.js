...
export class CoCActor extends Actor {
  ...
  /**
   * @name isWeakened
   * @returns true si l'active Effect Affaibli (radiation) et Immobilisé (restrain) est actif pour la caractéristique passée en paramètre
   */
  isWeakened(stat) {
    const stats = this.getFlag("coc", "weakened")
    return stats && stats.includes(stat)
  }
