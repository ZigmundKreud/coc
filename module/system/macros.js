...
export class Macros {
    ...
    static rollStatMacro = async function (actor, stat, onEnter = "submit") {
        if(actor){
            ...
            await CoCRoll.skillRollDialog(actor, game.i18n.localize(statObj.label), statObj.mod, 0, 20, statObj.superior, onEnter);
        } else {
    ...
    static rollItemMacro = function (itemId, itemName, itemType) {
        ...
        if (!item) return ui.notifications.warn(`${game.i18n.localize("COC.notification.MacroItemMissing")}: "${itemName}"`);
